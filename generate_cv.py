...
class CVGenerator:
    ...
    def draw_text_element(self, element: Dict[str, Any]) -> bool:
        ...
        try:
            self.canvas.setFont(font_name, element.get("size", 10.0))
        except Exception:
            # Font not available, use Helvetica fallback
            self.canvas.setFont("Helvetica", element.get("size", 10.0))
        
        # Draw text at specified position
        self.canvas.drawString(element.get("x", 0), element.get("y", 0), text)
